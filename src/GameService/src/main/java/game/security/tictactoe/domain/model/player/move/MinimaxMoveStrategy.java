...
public class MinimaxMoveStrategy implements MoveStrategy {
    ...
    @Override
    public void makeMove(@NotNull Game game, @NotNull CellType side) {
        final Position p = findBestMove(game.getGameField(), side);

        if(p.isValid())
            game.getGameField().setCell(p.row(), p.col(), side);
    }

    private Position findBestMove(GameField field, CellType side) {
